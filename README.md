...
```
Facebook Comments (Scraper) → /update-knowledge → Clustering Pipeline → daily_clusters.txt
                                                                              │
...
3. If there's no cache hit, **Qwen** is lazy-loaded (8-bit quantized) if it isn't already in memory.
...
│   └── qwen_engine.py        # Loads Qwen (8-bit) + drives the 3-stage pipeline (Rewrite→Route→Answer)
│
...
- `bertopic`, `umap-learn`, `hdbscan`, `scikit-learn` — clustering
- `transformers`, `accelerate` — running Qwen
...
### 1. With Docker
```bash
docker build -t sports-rag-platform .
docker run --gpus all -p 8000:8000 sports-rag-platform
```
